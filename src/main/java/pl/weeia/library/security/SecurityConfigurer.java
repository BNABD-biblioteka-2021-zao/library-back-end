...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .permitAll()
                .antMatchers(API+"/auth/refresh-token",API+"/test/nice", API+"/user")
                .authenticated()
                .antMatchers(API+"/test/user", API+"/book/all", API+"/borrowing")
                .hasRole("USER")
                .antMatchers(API+"/test/librarian", API+"/book/**", API+"/book/all", API+"/bookcopy/**", API+"/borrowing/**", API+"/borrowing", API+"/user/all")
                .hasRole("LIBRARIAN")
                .anyRequest()
                .authenticated()
                .and()
